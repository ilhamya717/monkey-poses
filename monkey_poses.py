...
def classify_four(lms_np, handed_label, face_info):
    ...
    thumb_up_vert = (lms_np[4][1] < lms_np[3][1] and
                     lms_np[4][1] < lms_np[2][1])
    fingers_folded = (not states["index"] and not states["middle"]
                      and not states["ring"] and not states["pinky"])
    oke = thumb_up_vert and fingers_folded

    # Priority: OKE → INDEX_NEAR_FACE → MIDDLE_UP → INDEX_UP → IDLE
    if oke:        return "OKE"
    if near_face:
...
def open_camera(index=0):
    for idx in (index, 1 - index):
        ...
        for _ in range(8):
            ...
            time.sleep(0.05)
        if ok:
            label = "DSHOW" if sys.platform == "win32" else "AVFOUNDATION"
            print(f"[camera] using index {idx} ({label})")
            return cap
        cap.release()
...
def build_panel_cache(panel_size):
    ...
    for g in ["INDEX_NEAR_FACE", "MIDDLE_UP", "INDEX_UP", "IDLE", "OKE"]:
        if g not in cache:
...
def main():
    ...
    # 
    win_cam   = "Monkey_Poses — [q/ESC] quit  [C] switch  [0/1] jump  [H] panel  [R] reload"
    win_panel = "Monkey Poses"
    ...
    with mp_hands.Hands(
        ...
        while True:
            ...
            if result.multi_hand_landmarks and result.multi_handedness:
                ...
                wrist_xy = all_lms_np[0][WRIST_ID][:2].astype(int)
                h_label  = result.multi_handedness[0].classification[0].label
                draw_label(frame, f"{h_label}: {stable_gesture}",
            ...
            cv2.putText(frame, "BUILD: Monkey_Poses", (10, 90),
